...
**Next.** Back-adjusted continuous contracts; returns, realized volatility, volume and
...
scripts/               estimate_cost, fetch_data, build_roll_calendar
...
contracts trade on the roll session, so the back-adjustment gap will use two same-day
closes and no future information. The full data caveats are in
